...
def calculate_metrics():
    ...
    tbl.header(["ID", "Sent", "AMR", "UCCA", "INT", "UD"])

    k = 0
    for sent in lines:
        tbl.add_row([1 + k, sent, metrics["amr"][k], metrics["ucca"][k], metrics["intp"][k], metrics["ud"][k]])
        k = k + 1
    tbl.add_row(["", "TOTAL SCORE", get_score(metrics["amr"]), get_score(metrics["ucca"]), get_score(metrics["intp"]),
                 get_score(metrics["ud"])])
